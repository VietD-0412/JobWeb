...
const contentManagementTab = document.getElementById('tab-content-management');
const userProgressContent = document.getElementById('content-user-progress');
const contentManagementContent = document.getElementById('content-content-management');

// --- CONTENT MANAGEMENT FORM ELEMENTS ---
...
// --- TAB SWITCHING LOGIC ---
const setupTabs = () => {
    userProgressTab.addEventListener('click', (e) => {
        e.preventDefault();
        userProgressContent.classList.remove('hidden');
        contentManagementContent.classList.add('hidden');
        userProgressTab.classList.add('text-indigo-600', 'border-indigo-500');
        userProgressTab.classList.remove('text-gray-500', 'border-transparent', 'hover:text-gray-700', 'hover:border-gray-300');
        contentManagementTab.classList.add('text-gray-500', 'border-transparent', 'hover:text-gray-700', 'hover:border-gray-300');
        contentManagementTab.classList.remove('text-indigo-600', 'border-indigo-500');
    });

    contentManagementTab.addEventListener('click', (e) => {
        e.preventDefault();
        contentManagementContent.classList.remove('hidden');
        userProgressContent.classList.add('hidden');
        contentManagementTab.classList.add('text-indigo-600', 'border-indigo-500');
        contentManagementTab.classList.remove('text-gray-500', 'border-transparent', 'hover:text-gray-700', 'hover:border-gray-300');
        userProgressTab.classList.add('text-gray-500', 'border-transparent', 'hover:text-gray-700', 'hover:border-gray-300');
        userProgressTab.classList.remove('text-indigo-600', 'border-indigo-500');
    });
};
...
const setupContentManagementForms = () => {
    // Handle the custom file input for videos
    if (videoFileInput && videoFileNameSpan) {
        videoFileInput.addEventListener('change', () => {
            if (videoFileInput.files.length > 0) {
                videoFileNameSpan.textContent = videoFileInput.files[0].name;
            } else {
